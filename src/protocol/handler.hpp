...
public:
    ...
    uint32_t data_read_token(int unique_id) { return storage_[unique_id].read_part.data_version(); }

    void wait_reading_data(int unique_id, uint32_t old_token) {
        storage_[unique_id].read_part.wait(old_token);
    }

    template <typename T>
    T data(int unique_id) {
        return storage_[unique_id].read_part.read<T>();
    }
