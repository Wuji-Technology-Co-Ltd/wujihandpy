...
| :--- | :--- | :--- | :--- |
| `WUJI_LOG_PATH` | Any directory path | `~/.wuji/log/` | Log file directory |
| `WUJI_LOG_LEVEL` | `trace` / `debug` / `info` / `warn` / `err` / `critical` / `off` | `info` | Log level |
| `WUJI_LOG_TO_FILE` | `true` / `false` | `true` | Write to file |
| `WUJI_LOG_TO_CONSOLE` | `true` / `false` | `true` | Write to terminal (stdout / stderr) |
